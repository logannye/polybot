...
async def main():
    ...
    ensemble = EnsembleAnalyzer(anthropic_key=settings.anthropic_api_key, openai_key=settings.openai_api_key, google_key=settings.google_api_key)
    wallet = WalletManager(private_key=settings.polymarket_private_key)
    executor = OrderExecutor(scanner=scanner, wallet=wallet, db=db, fill_timeout_seconds=settings.fill_timeout_seconds)
    recorder = TradeRecorder(db=db)
    risk_manager = RiskManager(
        max_single_pct=settings.max_single_position_pct, max_total_deployed_pct=settings.max_total_deployed_pct,
        max_per_category_pct=settings.max_per_category_pct, max_concurrent=settings.max_concurrent_positions,
        daily_loss_limit_pct=settings.daily_loss_limit_pct, circuit_breaker_hours=settings.circuit_breaker_hours,
        min_trade_size=settings.min_trade_size, book_depth_max_pct=settings.book_depth_max_pct,
    )
    engine = Engine(db=db, scanner=scanner, researcher=researcher, ensemble=ensemble, executor=executor,
                    recorder=recorder, risk_manager=risk_manager, settings=settings)
    app = create_app(db)
    dashboard_server = uvicorn.Server(uvicorn.Config(app, host="127.0.0.1", port=8080, log_level="warning"))
    try:
